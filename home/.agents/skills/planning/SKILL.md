...
Follow `../CODEX_COMPATIBILITY.md`. In Codex Plan mode, final plans must be
returned in a `<proposed_plan>` block. Use `request_user_input` only for material
choices that cannot be resolved by inspection.
