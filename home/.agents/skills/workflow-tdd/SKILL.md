...
Follow `../CODEX_COMPATIBILITY.md`. In Codex, run TDD locally by default. Use
`spawn_agent` only when the user explicitly asks for sub-agents, delegation, or
parallel agent work. `AskUserQuestion` means Plan mode `request_user_input` or a
concise conversational confirmation.
...
Codex では原則としてローカルに実装する。ユーザーが明示的に委譲を求めた場合のみ
`spawn_agent` で実装担当の sub-agent を起動する:
...
- ユーザーが明示的に委譲を求めた場合のみ、GREEN Phase の最小実装に対応する Codex sub-agent を使う。

## Error Handling

- **RED Phase でテストが通ってしまう**: テスト条件を見直し、未実装部分を正しく検証するテストに修正
- **GREEN Phase で最小実装が困難**: 機能をさらに小さな単位に分解し、段階的に実装
- **REFACTOR Phase でテストが壊れる**: `git diff` で変更を確認し、ユーザー確認後に取り消しまたはより小さな変更単位での再試行を提案
- **サイクル全体が行き詰まった場合**: ユーザーに以下の選択肢を提示:
  1. 現在のサイクルを小さな単位に再分解
