...
Follow `../CODEX_COMPATIBILITY.md`. In Codex, `AskUserQuestion` means an explicit
user confirmation in the current conversation. Never run `git commit` until the
user confirms the exact grouping and message.
...
Codex の現在の会話でインライン確認（対話を終了せず継続）:

1. **分類確認**: グループ一覧を表示後、ユーザーに確認
   - 選択肢: 「このまま実行」「グループを統合」「グループを分割」「キャンセル」
   - 「Other」で詳細な修正指示も可能

2. **コミット確認**: 各コミットメッセージ表示後、ユーザーに確認
