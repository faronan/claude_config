...
Follow `../CODEX_COMPATIBILITY.md`. In Codex, only run empirical evaluation when
the user explicitly authorizes sub-agents, because `spawn_agent` is the Codex
replacement for Claude Code's `Agent` tool. If delegation is not authorized,
perform structure review only and label it as non-empirical.
...
Codex では、ユーザーが sub-agent / 委譲 / 並列作業を明示的に求めた場合のみ
`spawn_agent` で **新規 subagent を dispatch** する。自己再読で代替しない。
...
- **指示側メトリクス**:
  ...
  - ステップ数: sub-agent 戻り値の usage メタが得られる場合は `tool_uses`
...
新規 subagent を dispatch できない環境（既に subagent として動作中、ユーザーが委譲を許可していない等）では、本 skill は **適用しない**。

- 代替案 1: 親セッションのユーザーに別セッションでの依頼を促す
...
- 詳細は references/ を参照:
  - subagent 起動契約テンプレ: この skill ディレクトリの `references/subagent-contract.md`
  - 失敗パターン台帳: この skill ディレクトリの `references/failure-pattern-ledger.md`
  - Red flags / よくある失敗 / バリアント探索: この skill ディレクトリの `references/red-flags.md`
- 関連 skill:
