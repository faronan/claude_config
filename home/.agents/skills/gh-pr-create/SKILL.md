...
and do not create or push a PR without explicit user confirmation.

<!-- codex-requires-confirmation: gh-pr-create -->
...
テンプレートが見つからない場合はこの skill ディレクトリの `template.md` を参照する。
...
- 本文: **なぜ**この変更が必要かを重視
- テンプレート集: この skill ディレクトリの `template.md` 参照（リポジトリにテンプレートがない場合のフォールバック）
