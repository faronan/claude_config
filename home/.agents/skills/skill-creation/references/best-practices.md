...
- `disable-model-invocation: true` — ユーザーが直接呼び出すワークフローに使用
  - Codex 用 skill ではこの Claude Code 固有フィールドに依存しない。自動発動を避けたい場合は description / trigger を狭め、本文で Codex Runtime の制約を明記する。
...
- `allowed-tools`: 使用ツール
- `context`: コンテキスト設定
- `agent`: 委譲先エージェント（Codex 用 skill では依存しない）
- `disable-model-invocation`: モデル起動の無効化（Codex 用 skill では依存しない）
