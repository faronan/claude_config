...
- **Codex 用 skill**: Claude Code のツール名は権限としては機能しない。`CODEX_COMPATIBILITY.md` の読み替え規約に従う
...
- **型**: string
- **用途**: Claude Code でエージェントに処理を委譲
- **例**: `agent: security-reviewer`
- **Codex 用 skill**: 未対応。Codex ではこのフィールドに依存せず、ユーザーが明示的に委譲を求めた場合のみ `spawn_agent` を使う

### disable-model-invocation（任意）

- **型**: boolean
- **用途**: Claude Code で `true` にするとワークフロー型スキルのモデル起動を無効化
- **使い分け**: 複雑なワークフロー → `true`、シンプルなスキル → 省略
- **Codex 用 skill**: 未対応。このフィールドに依存せず、description / trigger と本文の Runtime 節で制御する
...
5. **agent**: Claude Code 用 skill では `agents/` ディレクトリに対応ファイルが存在すること。Codex 用 skill では使用しない
