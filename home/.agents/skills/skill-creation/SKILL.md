...
Follow `../CODEX_COMPATIBILITY.md`. When creating Codex-facing skills, avoid
Claude Code-only tool names in the body unless they are explicitly documented as
compatibility notes.
...
- **スキル名が未指定**: ユーザーにスキルの目的を確認
- **同名スキルが既存**: 上書きするか別名にするかを確認
- **allowed-tools に不明なツール**: 利用可能なツール一覧を提示

## Notes

- 詳細なベストプラクティスはこの skill ディレクトリの `references/best-practices.md` を参照
- frontmatter の仕様はこの skill ディレクトリの `references/frontmatter-spec.md` を参照
