...
effort: medium
argument-hint: "[target spec or feature]"
description: |
  Playwright Test (E2E) のベストプラクティスとリファレンス。
  テスト執筆、固定 wait 回避、ネットワーク待機、ロケーター戦略、認証再利用、
  GitHub Actions での shard / retry / browser matrix 設定をカバー。
  Use when the user asks to write or fix Playwright E2E tests, or set up Playwright CI.
  Trigger words: "Playwright", "E2E", "playwright.config", "page.goto", "getByRole",
  "shard", "trace", "headed", "codegen", "storageState",
  "E2Eテスト", "ブラウザテスト", "エンドツーエンド".
allowed-tools:
  - Read
  - Glob
  - Grep
...
Original: <https://github.com/mizchi/skills/blob/main/playwright-test/SKILL-ja.md>

## Package Manager

ユーザーの toolchain は pnpm デフォルト。本文中の `npx playwright …` は `pnpm exec playwright …` に置き換え可（lockfile に従う）。CI ワークフロー例は upstream に合わせて `npm ci` / `npx playwright` のままにしてある — repo 側の lockfile に応じて調整すること。
...
`webkit` プロジェクトが必要な場合は `projects` に追加する。
...
- **`waitForTimeout` を見つけたら**: 自動待機 / web-first assertion に置換する（本文「鉄則: 固定 wait を使わない」を参照）
- **flaky テスト**: one-shot API (`isVisible()` / `count()` 等) を `expect(locator).toX()` に置換できないか確認
- **`waitForResponse` が timeout する**: API がそもそも呼ばれない可能性。web-first assertion 単独で代替できないか先に検証
- **CI のみ失敗**: フォント未インストール / `workers: 1` 設定漏れ / `webServer.reuseExistingServer` の挙動差を確認
- **トレース artifact が肥大**: `trace: 'on'` を `'on-first-retry'` に変更
